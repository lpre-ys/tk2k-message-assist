...
var JsSerializer = function () {
  function JsSerializer(config) {
    _classCallCheck(this, JsSerializer);
  ...
  _createClass(JsSerializer, [{
      ...
      messageBlockList.forEach(function (messageBlock) {
        ...
        messageBlock.messageList.forEach(function (message) {
          ...
          var line = message.line.map(function (text) {
            var _extractCenter2 = _this2._extractCenter(text, showFace, inCenter);

            var padding = _extractCenter2.padding;
            var inner = _extractCenter2.inner;
            var nextInCenter = _extractCenter2.nextInCenter;

            inCenter = nextInCenter;
            return padding + _this2._toTbScript(inner);
          });
      ...
      parts = parts.map(function (part) {
        if (/^<[a-z0-9\-_ ='"]+>$/.test(part)) {
          ...
          if (tagName === 'speed') {
            ...
            _this3.speedStack.push(prevSpeed);
          ...
          if (!_this3.config.getColorNumber(_tagName) === false) {
            ...
            // スピードタグ
            prevSpeed = _this3.speedStack.pop();
    ...
    value: function _extractCenter(text, hasFace) {
      var inCenter = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];
      ...
      if (fullMatch) {
        ...
        var lead = _fullMatch[1];
        var inner = _fullMatch[2];
        var trail = _fullMatch[3];
      ...
      if (!inCenter && !text.includes('</center>')) {
        ...
        if (startMatch) {
          ...
          var _lead = _startMatch[1];
          var afterCenter = _startMatch[2];
      ...
      if (inCenter) {
        ...
        if (endMatch) {
          ...
          var beforeCenter = _endMatch[1];
          var _trail = _endMatch[2];
