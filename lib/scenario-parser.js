...
var ScenarioParser = function () {
  ...
  _createClass(ScenarioParser, [{
    ...
    value: function serialize() {
      var isJs = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
    ...
    value: function _textParse(textList) {
      ...
      textList.forEach(function (text) {
        ...
        if (_this3.config.hasFace && faceCommandRegExp.test(text)) {
          ...
          var se = null;
          var seMatch = faceCommand.match(/\s+se=(\S+)$/i);
          if (seMatch) {
            se = seMatch[1];
    ...
    value: function _parseSystemTag(pbTag) {
      ...
      var systemConfig = this.config.getSystem(name);
      if (!systemConfig) {
        throw new Error('\u5B58\u5728\u3057\u306A\u3044system\u3067\u3059: ' + name);
      }
      return new _system2.default(name, systemConfig);
    }
  }, {
    key: '_tagFormat',
    value: function _tagFormat(textList, comments) {
      ...
      if (stack.length > 0) {
        var prev = '';
        this.continueTag = stack.filter(function (v) {
          if (prev != v) {
            prev = v;
            return true;
          }
